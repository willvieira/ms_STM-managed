###############################
# Script to plot calculated shift as boxplot (fig supp 4)
# Will Vieira
# Feb, 2021
##############################
...
# Fig supp 4
    ...
    png(filename = paste0('manuscript/img/sim-result_supp4.png'), width = 5.5, height = 5.5, units = 'in', res = 250)
    par(mfrow = c(2, 1), mar = c(1, 2.8, 1.2, 0.8), oma = c(.6, 0, .5, 0), mgp = c(1.2, 0.2, 0), tck = -.008, cex = 0.8)

    # define y axis limits`
    yLim <- range(subset(summ_dt2, sim %in% c('T150+FM', 'T150+CC+FM') & RCP %in% c(0, 8.5))$diff)
