...
**Running title**: Forest management accelerates the response of forest to climate change
...
This model describes metapopulation as a set of patches tha are either occupied or empty, and connected by dispersal.
...
We can further extend this model to incorporate enviromental gradient by turning the demographic parameters ($\alpha$ and $\varepsilon$) as a function of climate conditions.
...
Further than the colonization ($\alpha$) and extinction ($\varepsilon$) processes driving the transitions between empty (R) and occupied (by either B, M or T) patches, the model descibes species interaction through the mechanisms of succession and competitve exclusion.
...
This time discrete Markok Chain approach is dynamic as each parameter is calculted based on previous enviromental conditions (annual mean temperature and annual precipition) only, and the transitions are dependent on the neighbourhood proportion (Figure \@ref(fig:model)).

![Schema of the State and Transition Model (@Vissault2020) with integrated forest management practices. Directional arrows describe the possible transitions between the four forest states: (R)egeneration, (B)oreal, (T)emperate and (M)ixed. For each arrow, equations represent how the transition between states are calculated, with parameters changing in function of mean annual temperature and precipitation. In red are the parts of the equation describing the natural dynamics, in which are reduced when the respective management practice is applied. The values of each of the 9 parameters are shown in the supplementary figure 4.](img/model_equation_fm.png){#fig:model}
...
For each plot and each measure (ranging between 1960 and 2010 with median of 5 years), the forest states (B, M and T) were classified following its species composition.
Temperate composition was classified with the presence of at least one of the following 7 species: *Prunus serotina*, *Acer rubrum*, *Acer saccharum*, *Fraxinus americana*, *Fraxinus nigra*, *Fagus grandifolia*, *Ostrya virginiana*, *Tilia americana*; while boreal with at least one of the following 8 species: *Picea mariana*, *Picea glauca*, *Picea rubens*, *Larix laricina*, *Pinus banksiana*, *Tsuga canadensis*, *Abies balsamea*, *Melia azedarach*.
When at least one species from boreal and temperate compositions were found together, the stand was classified as mixed.
Regeneration plot was classified when the basal area was inferior than 1m$^2$ ha$^{-1}$.
Transition probabilities (scaled to respect the same time interval to all plots) was estimated with logistic regression (GLM) based on climate variables.
Importantly, the use of extensive forest inventory data to calibrate the parameters allowed the model to predict with good confidence (*Je n'arrive pas à trouver un valuer quantitative dans l'article de Steve*) the distribution of the boreal, mixed and temperate states using current enviromental data.

## Adapted forest management: reducing the gap between potential and actual forest distribution

It is expected with warming temperature that the distribution of the boreal-temperate ecotone will shift northward, however due to slow demographic rates and limited dispersal of forest trees, these forest composition may lag behind climate change [@Talluto2017; @Vissault2020].
...
The following sections details the ecological assumptions and mathematical formulation for each practice implemented in the model.

### Managing to reduce colonization credit

Colonization of temperate species beyond the leading edge of their distribution may depend on many factors such as climate conditions, competitive ability and seed source.
...
Once environmental limitation is relaxed with warming temperature, species interactions such as competition for light may limit the development of regenerating individuals [e.g. @Bianchi2018].
Finally, seed source is a density-dependent processes in which, associated with the slow migration rate of trees, contributes to the lack of colonization beyond the population range limits.
In the context of managing ecological processes, some of these factors can be modified with forest management.
Here we choose two management practices that may act at different spatial scales to simulate density-independent colonization processes: plantation (i.e. assisted migration) at the large spatial scale, and enrichment planting at the local spatial scale.
...
Plantation is modelled as an increase of the probability of regeneration stands to become temperate $P(T|R)$.
A proportion $p$ of available regeneration stands are directly converted into temperate forest.
...
Different ecological mechanisms can explain extinction debt emerged from the delayed response of forest trees to warming temperature.
Slow demographic rates along with dispersal limitations can delay the reponse of species to environmental changes [@Dullinger2012].
...
Similarly, thinning of boreal species in mixed stands can simulate an increase in the fitness of temperate species which then is able to competitive exclude boreal species.
Both harvest and thinning are intent to open space and reduce the proportion of boreal species, and  therefore increase the likelihood of temperate species to shift northward.

#### Harvest of boreal stands

In the natural extinction model, boreal stands becomes in a regeneration state only after disturbances occuring at a rate ($\varepsilon$).
Harvest is modelled as an increase of the probability of boreal stands to become regeneration $P(R|B)$.
...
The exclusion of boreal trees by temperate trees in mixed stands is a function of instability of the mixed stand $\theta$ and the ratio of competitive abilites between temperate and boreal species, $\theta_T$.
...
The rationale is that the proportion $s1$ of mixed stands that are managed this way are directly converted into temperate.
...
While temperature varied, annual mean precipitation was fixed to 998.7 mm as precipitation has relatively small effects on the model compared to temperature [@Vissault2020].
...
Positive values of $-R_0$ indicate smooth transition to the new equilibrium whereas negative values indicate reactivity, *i.e.* an initial amplification against final equilibrium.
...
The fourth metric is the return time ($\Delta_{time}$) or ecosystem sensitivity, in which is estimated by the number of steps of the transitory phase, where each time step of the model is equal to 5 years.
The last metrics is cumulative amount of changes of the transitory phase, or ecosystem vulnerability [@Boulangeat2018].
It is defined as sum of all changes in the states after temperature warming, and is obtained by the integral of the states change over time ($\int S(t)dt$).
These five metrics together are able to summarize the multidimensionality of the response of a system to external disturbances.
...
For the first approach in which we tested the effect of forest management on the metrics characterizing the transient dynamics, we used five distinct scenarios: natural dynamics without forest management, 0.25% of plantation, 0.25% of enrichment planting, 1% of harvest and 0.25% of thinning, in an annual rate.
...
To our knowledge, enrichment planting and thinning are rarely used in Quebec, and so we choose to set the same amount as of plantation.
To further quantify the effect of increasing the intensity of forest management from 0 to 100% for each practice, we choose two locations from the gradient of temperature in which forest management had the most effect on the metrics of transient dynamics: -1 and 0$^{\circ}$C of annual mean temperature which represents the limit between the boreal and mixed region.
To overcome the multidimensionality of the simulations, we developped an online application to quantify the five metrics of the transient dynamics for any location of the temperature gradient, using any intensity of forest management, with three scenarios of warming temperature: https://ielab-s.dbio.usherbrooke.ca/STM-managed.

For the second approach testing the effect of forest management on the range shift of the boreal-temperate ecotone, we ran three simulations to compare the relative importance of warming temperature, forest management practices, and their interaction with the equilibrium distribution in future climate conditions.
The intensity of the four management practices were the same as used in the first approach.
The model ran for 150 years under three different scenarios: (i) only climate change, (ii) only one practice of forest management, and (iii) climate change and one practice of forest management at time.
These three simulations were then compared with current ($T_0$) and future ($T_1$) forest distribution at equilibrium with the climate conditions.
...
Plantation and enrichment planting reduced by about 40 and 80% at around -1.5$^{\circ}$C the time for forest to reach the new equilibrium after warming temperature, respectively (fig \@ref(fig:num-res1)e).
...
In the boreal region, plantation and enrichment planting had non-linear response in most of the cases; while in the mixed region, enrichment planting and thinning had non-linear response.
...
Surprisingly, thinning of boreal species had a negative effect reducing asymptotic resilience and increasing sensitivity at the boreal/mixed transition region.
...
Using a spatially explicit model accounting for dispersal limitations of trees and stochastic dynamics, we tested how forest management affect the range limit shift of the boreal trailing edge and the temperate + mixed leading edge (figure \@ref(fig:sim-result)).
...
We developed a simple and informative theoretical framework based on the metapopulation theory that allowed us to (i) establish the link between forest management and ecological mechanisms, and to (ii) investigate the effect of forest management on the response of the boreal-temperate ecotone to climate change.
...
It should help future managers to take into account both natural and anthropogenic perturbations on forest dynamics.

***Plantation and enrichment planting reduce colonization credit***
...
Our results suggest that plantation and enrichment planting of temperate species on the boreal region can reduce this lag by reducing the transient period and increasing the temperate northward range shift.
...
In the simulated transient dynamic, we found limited increase in asymptotic resilience when applying enrichment planting in the boreal-mixed transition region, which means a faster recover to equilibrium after disturbance (Figure \@ref(fig:num-res1)).
...
Enrichment planting of temperate trees into boreal areas had a stronger effect to both reduce the transient period and reduce colonization credit when compared with planting.
This is due to two different mechanisms.
First, the intensity of forest management in the model is relative to the state abundance; hence 0.25% of boreal stands being enriched is much higher than 0.25% of re stands being planted.
...
These results suggest that enrichment planting in local gaps have the best potential compared to plantation to help forest keep pace with climate change.
...
It means enrichment planting at local gaps is more effective, but this practce alone may not be as effective as expressed here as empty places are needed to perform this practice.
...
***Why does reducing colonization credit increases range shift but reducing extinction debt does not?***
...
Such lack of effect on range shift when reducing colonization credit may be explained by source-sink dynamics that, dispite the increase in the extinction rate of patches due to harvest, increases the likelihood of harvested boreal stands becoming boreal in the future.
Furthermore, limited dispersal contributes to lack of temperate colonization in harvested patches, which may explain the results found in @Brice2020 as they do not consider dispersion in their model.
...
Thinning indeed increased the proportion of temperate stands on the mixed region, because it helps competitive exclusion.
...
Such lack of temperate invasion on the boreal region may be explained by the difficulty of temperate to settle in boreal stands compared to regeneration stands, included in the model indirectly throught the invasion (mean $\beta_{T}$ = 0.62) and colonization (mean $\alpha_{T}$ = 0.99) parameters.
...
In our context, demographic models can test the effect of forest management in the growth, mortality and regeneration processes, and a stand-level model such as ours help better understanding how the effect of management practices scales up to a larger spatial scale.
We should also cautiously interpret the effect of climate change as simulated here.
Although is predicted that drought intensity will increase with climate change and it may drive how forest will respond to climate change, here we have simulated only warming temperature, while precipitation remained constant.
Some studies have shown tree species to be more sensitive to increase in drought rather than temperature [*e.g.* white spruce @Andalo2005].
...
The present study rather show how communities could adapt to a continuous change in the environment and how management could help.
...
However, we can expect that the spatial distribution of different practices would interact and change the final results.
For instance, creating gaps with selective harvest of boreal stands nearby the mixed distribution or the temperate tree plantations may create a synergy.
...
However, planting temperate trees after harvesting boreal stands may overcome the limitations of these two practices when applied alone, and therefore increase the nortward range shift.
...
We propose future studies should focus on the interaction between the management practices, and how the spatial distribution of these practices alter range limit shift.
...
![Figure S4: Boreal (left panels) and temperate plus mixed (right panels) occupancy along the latitudinal gradient of the boreal-temperate ecotone. Light and dark shaded areas are a reference of the state occupancy at equilibrium before and after warming temperature, respectively. Each line is a different simulation to differentiate the isolated and interacting effect of climate change (CC) and forest management (FM). The results are the mean of 15 replications. For the sake of simplicity, the confidence interval is omitted as we found little variation between replicates. Management intensity was set to 0.25% for plantation, thinning and enrichment planting, and 1% for harvest. Climate change scenario was the RCP 8.5.](img/sim-result_RCP8.5.png){#fig:sim-result-supp4}

![Figure S5: Parameters of the State and Transition Model varying in function of annual mean temperature from @Vissault2020. Annual mean precipitation is fixed to 998.7 mm. Parameters for (b) before and after warming temperature following (c) RCP4.5 and (d) RCP8.5 climate change scenarios over the same latitudinal position.](img/num-result_supp2.png){#fig:sim-result-sup5}